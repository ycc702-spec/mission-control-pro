...
async function fetchMarketData() {
    try {
        ...
        applyQuote('taiex-value', 'taiex-change', d.taiex);
        applyQuote('sp500-value', 'sp500-change', d.sp500);
        ...
        applyQuote('vti-value', 'vti-change', d.vti);
        applyQuote('vxus-value', 'vxus-change', d.vxus);
        
        // Portfolio cards
        applyQuote('portfolio-voo', 'portfolio-voo-change', d.voo);
        applyQuote('portfolio-qqq', 'portfolio-qqq-change', d.qqq);
        applyQuote('portfolio-vti', 'portfolio-vti-change', d.vti);
        applyQuote('portfolio-vxus', 'portfolio-vxus-change', d.vxus);
    } catch (e) {
...
function updateGreeting() {
    ...
    if (hour >= 12 && hour < 18) greeting = 'Good afternoon, Commander';
    else if (hour >= 18) greeting = 'Good evening, Commander';
...
async function fetchWeather() {
    try {
        // 使用 Open-Meteo (free, no API key required)
        const lat = 24.8136, lon = 120.9555; // Hsinchu, Taiwan
        ...
        if (weatherTempEl) weatherTempEl.textContent = Math.round(current.temperature_2m) + '°C';
...
// ===== UPLOAD PAGE ACCESS CODE =====
function initUploadAccess() {
    const accessOverlay = document.getElementById('uploadAccessOverlay');
    const uploadInterface = document.getElementById('uploadInterface');
    const accessBtn = document.getElementById('uploadAccessBtn');
    const accessInput = document.getElementById('uploadAccessCode');
    ...
    accessBtn.addEventListener('click', () => {
        ...
        if (code === 'pupu') {
            ...
            accessInput.value = '';
            accessInput.placeholder = 'Wrong code. Try again.';
            setTimeout(() => {
...
function initUploadFunctionality() {
    ...
    dropZone.addEventListener('dragover', (e) => {
        e.preventDefault();
        dropZone.style.borderColor = '#0abd e3';
        dropZone.style.backgroundColor = 'rgba(10, 189, 227, 0.1)';
    });
